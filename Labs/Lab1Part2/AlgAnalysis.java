...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		int intervalle = maxSize/count;


		for (int i = intervalle; i<=maxSize; i += intervalle){
			long result = Long.MAX_VALUE;
			for (int j = 0; j<10; j++){
				int[] randArray = randomizeArray(genArray(i));
			...
			double m = ((double)result/(i*i));
			double f = ((double) result / (i * (Math.log(i) / Math.log(2))));

			System.out.println("N: "+i+", T(n): "+result+"nanosecs, T(n)/(n*n): "+m+", T(n)/(nlogn): "+f);
		}
	...
	public static long unique1Runtime(int n){
		int[] randArray = randomizeArray(genArray(n));
		long STime = System.nanoTime();
		Unique1.unique1(randArray);
		long ETime = System.nanoTime();
